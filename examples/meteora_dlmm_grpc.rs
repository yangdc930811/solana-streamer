//! Meteora DLMM subscription via gRPC.
//!
//! Usage: cargo run --example meteora_damm_grpc --release

use sol_common::common::constants::{METEORA_DAMM_V2_PROGRAM_ID, METEORA_DLMM_PROGRAM_ID};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("Meteora DLMM gRPC (solana-streamer)\n");
    ...
    let event_filter = EventTypeFilter {
        include: vec![EventType::MeteoraDlmmSwap2],
    };
    grpc.subscribe_events_immediate(
        vec![Protocol::MeteoraDlmm],
        None,
